...
module RISCV #(
    ...
    btb u_btb (
        ...
        .fetch_pc_i(pc_next),      // Look up what we are currently requesting
        .pred_taken_o(btb_pred_taken),
        .pred_target_o(btb_pred_target),
        .btb_hit_o(btb_hit),
        // Update (Writeback Stage)
        .update_en_i(btb_update_en),
        .update_pc_i(btb_update_pc),
        .update_target_i(btb_update_target),
        .update_actual_taken_i(btb_update_actual_taken)
    );
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            case (state)
                ...
                S_WRITEBACK: begin
                        // If the prediction was wrong, we must redirect
                        if ((branch || jump) && (taken_reg != pred_taken_q)) begin
                            redirect_valid <= 1'b1;
                            redirect_pc    <= taken_reg ? br_target : (pc_reg + 4);
                            inst_reg       <= 32'h00000013; // Flush
                        end
                        
                        // Always trigger the BTB update for branch/jump instructions
                        if (branch || jump) begin
                            btb_update_en           <= 1'b1;
                            btb_update_pc           <= pc_reg;
                            btb_update_target       <= br_target;
                            btb_update_actual_taken <= taken_reg;
                        end
                    end
            endcase
        end
