module lsu_unit #(
    ...
    // Handshake
    input  logic                  ready_i, // Grant from CDB arbiter
    ...
    // Debug for Reads (capture at Cycle 0)
    always_ff @(posedge clk) begin
        if (state == S_IDLE && valid_i && !mem_write_i) begin
            // We can't see the data *fetched* yet (it's being fetched this cycle)
            // But we can log the address
    ...
    always_comb begin
        ...
        case (state)
            ...
            S_WB: begin
                ...
                // Only return to IDLE if the CDB accepted us (Handshake)
                if (ready_i) begin
                    next_state = S_IDLE;
                end
            end
